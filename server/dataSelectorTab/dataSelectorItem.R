...
dataSelectorTabPanelObserve <- function(input,output,session,
    ...
    if(dbExistsTable(bookmarks, "Bookmarks")){
        ...
        currentMetadata$urlDF <- tmpUrlDF[, Timestamp := as.POSIXct(Timestamp, origin="1970-01-01 00:00")]
    } else {
    ...
    observeEvent(input$deleteBM,{
        if (!is.null(input$urlTable_rows_selected)) {
            dataSelectorMessages <- updateMessages(
                ...
                type="SUCCESS",
                msg=paste(getTime("SUCCESS"),
                    "Bookmark(s) ",paste(currentMetadata$urlDF$Description[as.numeric(input$urlTable_rows_selected)], collapse=", ")," removed",sep="")
                )
